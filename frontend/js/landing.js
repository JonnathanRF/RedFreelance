    ...
    try {
        ...
        if (data.length === 0) {
            ...
            data.forEach(categoryData => {
                ...
                `;
                // Puedes añadir un evento click para redirigir a una página de servicios filtrados por categoría
                categoryCard.querySelector('button').addEventListener('click', () => {
                    // Implementar redirección o filtrado aquí (ej: window.location.href = `services.html?category=${encodeURIComponent(categoryData.category)}`)
                    alert(`Navegando a servicios de ${categoryData.category}`);
                });
